...
class SellerLeadJob implements ShouldQueue
{
    ...
    public function handle()
    {
       ...
       $to_name4 = $this->data['name'];
        ...
        Mail::send('emails.sellerleadseth', ['data' => $this->data], function($message) use ($to_name, $to_email, $to_name2, $to_email2, $to_name3, $to_email3) {
            $message->to($to_email, $to_name)
                    ->to($to_email2, $to_name2)
                    ->to($to_email3, $to_name3)
                    ->bcc('max@tristarfinance.com')
                    ->subject('New Seller Lead Inquiry By: ' . $this->data['name']);
